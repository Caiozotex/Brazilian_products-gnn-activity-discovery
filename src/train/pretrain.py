...
def pretrain_hiv(model, train_loader, optimizer, device,
                 val_loader=None, epochs=50,
                 save_dir="checkpoints/hiv",
                 patience=10, checkpoint_every=5):
    """
    Pretrain HIV binary classifier with validation, early stopping, and checkpointing.

    Args:
        model: HIVModel (output logits, shape (batch_size, 1))
        train_loader: DataLoader for training
        val_loader:   DataLoader for validation (if None, splits train_loader 80/20)
        epochs:       max epochs
        save_dir:     directory for checkpoints and best model
    ...
    # If no val_loader provided, split training data
    if val_loader is None:
        dataset = train_loader.dataset   # list of Data objects (or a Dataset)
        n = len(dataset)
        indices = np.random.permutation(n)
        split = int(0.8 * n)
        train_idx, val_idx = indices[:split], indices[split:]
    ...
    for epoch in range(1, epochs + 1):
        ...
        for batch in train_loader:
            batch = batch.to(device)
            optimizer.zero_grad()
            logits = model(batch.x, batch.edge_index, batch.edge_attr, batch.batch)  # shape (B,1)
            targets = batch.y.view(-1, 1)   # shape (B,1)
        ...
        # Validation
        val_loss = evaluate_loss_binary(model, val_loader, device)
        ...
        if val_loss < best_val_loss:
            ...
            best_epoch = epoch
            no_improve = 0
            # Save best model (full model and encoder separately)
            torch.save(model.state_dict(), os.path.join(save_dir, "best_model.pt"))
            torch.save(model.encoder.state_dict(), os.path.join(save_dir, "best_encoder.pt"))
            print(f" -> New best model (val loss {val_loss:.4f})")
        else:
...
def evaluate_loss_binary(model, loader, device):
    """Evaluate binary cross-entropy loss on a given loader."""
    model.eval()
    total_loss = 0.0
    with torch.no_grad():
        for batch in loader:
