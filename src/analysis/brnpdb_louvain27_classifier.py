"""
Script 2 (Louvain 27): Calcula f(i) = hiv_active_i / (hiv_active_i + hiv_inactive_i)
...
Fonte stats : results/tables/brnpdb_louvain27_stats.csv
Fonte global: results/tables/louvain_node_communities.csv
Saída       : results/tables/brnpdb_louvain27_classification.csv
"""
import sys, io
...
STATS_CSV   = Path("results/tables/brnpdb_louvain27_stats.csv")
OUT_CSV     = Path("results/tables/brnpdb_louvain27_classification.csv")

# Enriquecimento mínimo em relação à taxa global para classificar como ativa
THRESHOLD = 2.0

def main():
    full = pd.read_csv(LOUVAIN_CSV)
    hiv  = full[full["is_brnpdb"] == False]

    n_active_global = int(hiv["is_active"].sum())
    n_hiv_total     = len(hiv)
    F_TOTAL         = n_active_global / n_hiv_total  # ≈ 0.0351
