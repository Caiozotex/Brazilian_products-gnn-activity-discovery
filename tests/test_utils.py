...
from src.utils.graph_utils import build_knn_similarity_graph, build_knn_similarity_graph_faiss, build_threshold_similarity_graph
...
def test_threshold_similarity_graph():
    emb = np.random.rand(30, 128)
    threshold = 0.95
    edge_index, N, edge_similarities = build_threshold_similarity_graph(emb, threshold=threshold)

    print(f"Threshold graph nodes: {N}")
    print(f"Threshold graph edge index shape: {edge_index.shape}")
    assert N == 30
    assert edge_index.shape[0] == 2
    for (i, j), sim in edge_similarities.items():
        assert sim >= threshold
    print("✓ full threshold similarity graph test passed!\n")
