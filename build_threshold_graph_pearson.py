...
def main():
    parser = argparse.ArgumentParser(description='Build a full threshold similarity graph using Pearson correlation.')
    ...
    parser.add_argument('--threshold', type=float, default=0.8, help='Pearson correlation threshold for edge creation')
    ...
    print('Computing full pairwise Pearson correlations...')
    from scipy.spatial.distance import pdist, squareform
    distances = pdist(all_emb, metric='correlation')
    sim_matrix = 1 - squareform(distances)  # Convert distance to similarity
    i_upper, j_upper = np.triu_indices(num_nodes, k=1)
    sim_values = sim_matrix[i_upper, j_upper]

    os.makedirs(args.save_dir, exist_ok=True)
    all_sim_path = os.path.join(args.save_dir, 'all_pairwise_pearson_similarities.csv')
    with open(all_sim_path, 'w') as f:
        f.write('node1,node2,pearson_similarity\n')
        for i, j, sim in zip(i_upper, j_upper, sim_values):
            f.write(f'{i},{j},{sim:.6f}\n')
    print(f'Saved all pairwise Pearson similarities to {all_sim_path}')

    print(f'Building full threshold similarity graph for {num_nodes} nodes with Pearson threshold {args.threshold}')
    valid = sim_values >= args.threshold
    edge_similarities = {}
    if valid.any():
    ...
    similarity_path = os.path.join(args.save_dir, f'edge_similarities_threshold_pearson_{args.threshold}.csv')
    df = pd.DataFrame([
    ...
    graph_path = os.path.join(args.save_dir, f'undirected_threshold_pearson_{args.threshold}_graph.pt')
    torch.save(graph, graph_path)
    print(f'Saved undirected threshold Pearson graph to {graph_path}')
